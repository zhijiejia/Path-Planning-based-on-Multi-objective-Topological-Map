...
int main() {
    ...
    for(int i = 1; i <= 1000000; i++)
	{
	 ...
	 double time_per = double(clock() - st)/CLOCKS_PER_SEC;
	st = clock();
    init();
    vector<Node> pop = pop_init(400);  //生成个体数量为400的种群
    GA ga = GA(pop, 400, 1.0, 1.0);
    int generation = 0;  //迭代次数
    ...
    ga.outputResult();
    puts("end");
    cout << (double(clock() - st)/CLOCKS_PER_SEC) / time_per * 0.03 << endl; 
